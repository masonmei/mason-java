...
@Consumes(MediaType.APPLICATION_JSON)
@Path("/get")
public Relation getRelation(@HeaderParam(Constants.REST_TOKEN_KEY) String token, @QueryParam("id") String id){
...
@Consumes(MediaType.APPLICATION_JSON)
@Path("/delete")
public boolean deleteRelation(@QueryParam("id") String id, @HeaderParam(Constants.REST_TOKEN_KEY) String token) {
	try {
		Account account = accountActivityManager.getAccount(token);
		if (account != null) {
